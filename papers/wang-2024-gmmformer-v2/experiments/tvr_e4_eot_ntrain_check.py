#!/usr/bin/env python3
"""CPU check for TVR E4 EOT readout: parent E4 hparams, CLIP EOT (1,512)."""
import os
import sys
...
os.environ["PRVR_ROOT"] = os.path.join(
    root, "tmp", "tvr_e4_ntrain_check_i3d_cliptext_e4_eot"
)
...
if "i3d_cliptext_e4_eot" not in cfg["root"]:
    raise SystemExit("FAIL root not eot %s" % cfg["root"])
...
txt = os.path.join(cfg["data_root"], "tvr", "TextData", "tvr_clip_B32_eot.h5")
zip_eot = os.path.join(cfg["data_root"], "tvr", "TextData", "clip_ViT_B_32_tvr_query_feat.hdf5")
print("eot", txt, os.path.isfile(txt), os.path.getsize(txt) if os.path.isfile(txt) else 0)
print("zip_eot_unused", zip_eot, os.path.isfile(zip_eot))
if not os.path.isfile(txt):
    raise SystemExit("FAIL missing CLIP eot h5")
...
with h5py.File(txt, "r") as h:
    ...
    if tuple(arr.shape) not in ((1, 512), (512,)):
        raise SystemExit("FAIL eot shape %s" % (arr.shape,))
    if dims != {512}:
        raise SystemExit("FAIL CLIP dim %s" % dims)

builder = open(os.path.join(src, "Datasets", "builder.py"), encoding="utf-8").read()
if "clip_B32_eot.h5" not in builder:
    raise SystemExit("FAIL builder missing eot h5")
if "i3d_cliptext_e4_eot" not in builder:
    raise SystemExit("FAIL builder missing eot PRVR_ROOT branch")
...
print("text_feat_path_hint", "clip_B32_eot.h5")
