#!/usr/bin/env python3
"""CPU check for TVR E4 sft=0.6: same I3D+CLIP as parent E4, only sft_factor changes."""
import os
import sys
...
os.environ["PRVR_ROOT"] = os.path.join(
    root, "tmp", "tvr_e4_ntrain_check_i3d_cliptext_e4_sft06"
)
...
if abs(float(cfg["sft_factor"]) - 0.6) > 1e-12:
    raise SystemExit("FAIL sft_factor=%s want 0.6" % cfg["sft_factor"])
...
if "i3d_cliptext_e4_sft06" not in cfg["root"]:
    raise SystemExit("FAIL root not sft06 %s" % cfg["root"])
...
for p, name in ((old_e4, "original E4"), (old_lr, "lr2e-4"), (old_i3d, "I3D TVR")):
...
print("E4_SFT06_NTRAIN_CHECK_OK")
