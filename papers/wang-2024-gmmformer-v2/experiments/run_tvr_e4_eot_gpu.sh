#!/bin/bash
# E4 TVR EOT readout only. Parent E4 lr=3e-4 sft=0.09. Do not overwrite orig/lr2e-4/sft06/I3D. Never GPU2.
set -euo pipefail
GPU="${1:?gpu id}"
SEED="${2:?seed}"
...
export PRVR_ROOT="$ROOT/tmp/tvr_i3d_cliptext_e4_eot_seed${SEED}"
...
if [ "$PRVR_ROOT" = "$OLD_E4" ] || [ "$PRVR_ROOT" = "$OLD_LR" ] || [ "$PRVR_ROOT" = "$OLD_SFT" ] || [ "$PRVR_ROOT" = "$OLD_I3D" ]; then
  echo "REFUSE_OVERWRITE $PRVR_ROOT" >&2
  exit 1
fi
if [ ! -f "$ROOT/data/prvr/tvr/TextData/tvr_clip_B32_eot.h5" ]; then
  echo "MISSING_EOT_H5" >&2
  exit 1
fi
CFG="$ROOT/vendor/GMMFormer_v2/src/Configs/tvr.py"
SRC_CFG="$ROOT/experiments/tvr_e4_eot_5090_1.py"
if [ ! -f "$SRC_CFG" ]; then
...
if ! grep -q 'tvr_i3d_cliptext_e4_eot_seed' "$CFG"; then
  echo "ROOT_NOT_EOT" >&2
  exit 1
fi
...
if ! grep -q 'clip_B32_eot.h5' "$BD"; then
  echo "BUILDER_NO_EOT_H5" >&2
  exit 1
fi
...
RUNDIR="$ROOT/logs/runs/tvr_e4_eot_seed${SEED}_gpu${GPU}"
LOCKDIR="$ROOT/logs/runs/tvr_e4_eot_seed${SEED}.lock"
...
if mkdir "$LOCKDIR" 2>/dev/null; then
  ...
  if pgrep -u zhaopu -f "PRVR_ROOT=$PRVR_ROOT" >/dev/null; then
    echo "TVR_E4_EOT_ALREADY_RUNNING" >&2
    exit 1
  fi
...
LN="$ROOT/logs/tvr_e4_eot_seed${SEED}_gpu${GPU}.log"
ln -sfn "$STDOUT" "$LN"
...
run = {
    ...
    "sft_factor": 0.09,
    "q_feat_size": 512,
    "text": "tvr_clip_B32_eot.h5",
    "started": time.strftime("%Y-%m-%dT%H:%M:%S"),
    "note": "E4 TVR I3D vis + CLIP EOT text, lr 3e-4 sft 0.09, not Table 1, no resume",
}
