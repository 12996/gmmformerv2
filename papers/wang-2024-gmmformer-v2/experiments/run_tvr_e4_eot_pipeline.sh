#!/bin/bash
# Extract EOT h5, patch builder, check, launch TVR E4 EOT train. Prefer GPU6. Never GPU2.
set -euo pipefail
ROOT=/data/zhaopu/wang-2024-gmmformer-v2
SEED=9527
WANT_GPU="${1:-6}"
PIPELOG="$ROOT/logs/tvr_e4_eot_pipeline.log"
mkdir -p "$ROOT/logs"
...
chmod +x "$ROOT/experiments/tvr_extract_clip_text_eot.sh" \
  "$ROOT/experiments/run_tvr_e4_eot_gpu.sh"

echo "====EXTRACT===="
bash "$ROOT/experiments/tvr_extract_clip_text_eot.sh" "$GPU"
if ! grep -q EXTRACT_EOT_OK "$ROOT/logs/tvr_e4_extract_eot.log"; then
  echo "BLOCKED extract failed"
  exit 4
fi

echo "====PATCH===="
"$ROOT/conda/bin/python" -u "$ROOT/experiments/patch_tvr_e4_eot_text.py"

CFG="$ROOT/vendor/GMMFormer_v2/src/Configs/tvr.py"
SRC="$ROOT/experiments/tvr_e4_eot_5090_1.py"
if [ ! -f "$CFG.bak_before_e4_eot" ]; then
  cp -a "$CFG" "$CFG.bak_before_e4_eot"
  echo BAK_OK
else
  echo BAK_KEEP
fi
...
test -f "$ROOT/data/prvr/tvr/TextData/tvr_clip_B32_eot.h5"
grep -q 'cfg\["sft_factor"\] = 0.09' "$CFG"
grep -q 'cfg\["lr"\] = 0.0003' "$CFG"
grep -q 'tvr_i3d_cliptext_e4_eot_seed' "$CFG"
! grep -q -- '--resume' "$CFG"
echo DEPLOY_OK

echo "====FAST_CHECK===="
"$ROOT/conda/bin/python" -u "$ROOT/experiments/tvr_e4_eot_ntrain_check.py" --fast
echo "====NTRAIN===="
"$ROOT/conda/bin/python" -u "$ROOT/experiments/tvr_e4_eot_ntrain_check.py"
...
RUN="$ROOT/logs/runs/tvr_e4_eot_seed${SEED}_gpu${GPU2}"
mkdir -p "$RUN"
nohup "$ROOT/experiments/run_tvr_e4_eot_gpu.sh" "$GPU2" "$SEED" > "$RUN/launcher.out" 2>&1 &
echo LAUNCHER_PID=$!
sleep 15
echo ====pid====
if [ ! -f "$RUN/pid" ]; then
...
echo ====ps====
ps -u zhaopu -o pid,etime,cmd | grep -E 'main.py -d tvr|run_tvr_e4_eot' | grep -v grep || echo NO_PS
...
python3 - <<'PY'
import os, yaml
p="/data/zhaopu/wang-2024-gmmformer-v2/tmp/tvr_i3d_cliptext_e4_eot_seed9527/results/tvr/gmmformer_v2/hyperparams.yaml"
...
keys=["lr","sft_factor","n_epoch","max_es_cnt","q_feat_size","visual_feature","root","hard_negative_start_epoch","batchsize"]
