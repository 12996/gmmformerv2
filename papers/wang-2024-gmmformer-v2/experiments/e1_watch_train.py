#!/usr/bin/env python3
"""Watch E1 Charades train on 5090_1 until Early Stop / 100 epoch / crash."""
...
dirs = sorted(glob.glob(root + "/logs/runs/tc_seed9527_gpu*"))
print("RUNDIRS", dirs)
...
for d in dirs:
    ...
    print("RUN gpu=%s pid=%s alive=%s exit=%s fail=%s loss=%s epoch=%s last_rsum=%s best=%s early=%s" % (
...
def classify(text: str) -> str:
    ...
    if m:
        val = float(m.group(1))
        # CLIP-random was ~9.6; healthy I3D first val was 11.6.
        if val <= 10.2:
            return "random_like"
...
def main() -> int:
    # first-epoch gate: wait up to 20 min for first val
    t0 = time.time()
    first_ok = False
    while time.time() - t0 < 20 * 60:
        ...
        time.sleep(30)
    if not first_ok:
        print("FAILED no first-epoch val within 20min", flush=True)
        return 1
